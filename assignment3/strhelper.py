...
def check_var(s:str,in_str:str = ""):
    if len(s) == 0: do_err("Empty var")
    if s[0] not in ALPHA: do_err("Var must start with a letter") # must start with letter
    # check if the var only contains legal characters stop at ( or )
    for c in s:
        if c == '(' or c == ')': break
        if c not in ALLCHAR: do_err("Illegal var name")
...
class Type: pass
class Type:
    def __init__(self, content: str = "", top:Type = None):
        ...
        if len(content) == 0:
            ...
            self.type = Type(content[1:len(content)-1],self)
        elif '->' not in content:
            check_var(content)
...
class Expression: # class used for making a binary tree of tokens, recursively
    def __init__(self, content: str = "", top:Expression = None, is_abstract:bool = False):
        ...
        if len(content) == 0: # end here if empty constructor
            ...
            elif content[closing_index+1:] == ':': ######### assign3
            ...
            if l_bracket_index != -1: # for for example: "expr(expr)"
                self.expr1 = Expression(content[0:l_bracket_index],self)
                self.expr2 = Expression(content[l_bracket_index:len(content)],self)
            else: # for for example: "expr expr"
                space_index = content.find(' ')
                self.expr1 = Expression(content[0:space_index],self)
                self.expr2 = Expression(content[space_index+1:len(content)],self)
        if not self.is_var:
...
def remove_excess_spaces(s:str) -> str:
    ...
    while i >= 0:
        if l[i] == '(' or l[i] == ':':
            ...
            if l[i+1] != '>': do_err("Missing > after \''-\'")
            is_bracket = True
        elif l[i] == '>':
            if l[i-1] != '-': do_err("Missing - before \''>\'")
            is_bracket = True
        elif l[i] == ' ':
            if is_bracket:
                l.pop(i)
