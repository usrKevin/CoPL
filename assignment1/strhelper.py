    ...
    def __init__(self, content: str = "", top:Expression = None, is_abstract:bool = False):
        ...
        if len(content) == 0: # end here if empty constructor
            ...
            if self.content[spl] == '\\':
                ...
                if spl2 == -1: # no other expression next to lambda
                    self.expr1 = Expression(content[spl:len(content)],self)
                else:
                    spl2 += spl # skip over the next ' '
            ...
            self.is_var = True
            check_var(content) # check if it's actually a legal var
            self.var = content
...
def print_expr_tree(prefix:str, expr:Expression, is_left:bool): # prints binary tree using recursion (for debug)
...
# converts subtree to a string recursively
